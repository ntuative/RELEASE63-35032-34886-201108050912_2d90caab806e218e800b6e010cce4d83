package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_382,this.clientWindowDestroyed);
            this.var_317 = param2;
            this._working = true;
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1438 *= 0.75;
         this.var_1437 *= 0.75;
         if(Math.abs(this.var_1438) <= 1 && Math.abs(this.var_1437) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_317 != null)
         {
            this.var_317(this.var_1438,this.var_1437);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_382,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
