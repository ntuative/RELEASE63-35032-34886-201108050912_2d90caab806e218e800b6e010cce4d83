package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_2121 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1370 = this.var_2121.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1370)
         {
            this.var_1370.procedure = this.onButtonClick;
         }
         this.var_1369 = this.var_2121.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1369)
         {
            this.var_1369.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_305 >= this.var_76.length - 1)
         {
            this.var_1369.disable();
         }
         else
         {
            this.var_1369.enable();
         }
         if(this.var_305 <= 0)
         {
            this.var_1370.disable();
         }
         else
         {
            this.var_1370.enable();
         }
      }
      ...
      {
         ++this.var_305;
         this.var_305 = Math.min(this.var_305,this.var_76.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_305;
         this.var_305 = Math.max(this.var_305,0);
         this.openPage();
      }
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_76[this.var_305][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_76[this.var_305][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_76[this.var_305][1]).value;
