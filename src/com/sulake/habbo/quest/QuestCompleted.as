package com.sulake.habbo.quest
{
   ...
   public class QuestCompleted implements IDisposable
   {
      ...
      private static const const_1059:int = 2000;
      
      private static const const_1470:int = 5;
      
      private static const const_1469:int = 31;
      ...
      private var var_126:QuestMessageData;
      
      private var var_628:Animation;
      ...
      public function prepare(param1:QuestMessageData) : void
      {
         ...
         if(this._window == null)
         {
            ...
            this.var_628 = this.var_50.getTwinkleAnimation(this._window);
         }
         this._window.findChildByName("catalog_link_txt").caption = this.var_50.localization.getKey("quests.completed.cataloglink." + this.var_126.activityPointType);
         var _loc2_:String = "quests.completed.reward." + this.var_126.activityPointType;
         this.var_50.localization.registerParameter(_loc2_,"amount",this.var_126.rewardCurrencyAmount.toString());
         this._window.findChildByName("reward_txt").caption = this.var_50.localization.getKey(_loc2_,_loc2_);
         this._window.visible = false;
         this._window.findChildByName("congrats_txt").caption = this.var_50.localization.getKey(!!this.var_126.lastQuestInCampaign ? "quests.completed.campaign.caption" : "quests.completed.quest.caption");
         this._window.findChildByName("more_quests_button").visible = this.var_126.lastQuestInCampaign;
         this._window.findChildByName("campaign_reward_icon").visible = this.var_126.lastQuestInCampaign;
         this._window.findChildByName("catalog_link_region").visible = !this.var_126.lastQuestInCampaign;
         this._window.findChildByName("next_quest_button").visible = !this.var_126.lastQuestInCampaign;
         this._window.findChildByName("reward_icon").visible = !this.var_126.lastQuestInCampaign;
         this._window.findChildByName("campaign_reward_icon").visible = this.var_126.lastQuestInCampaign;
         this._window.findChildByName("campaign_pic_bitmap").visible = this.var_126.lastQuestInCampaign;
         this.setWindowTitle(!!this.var_126.lastQuestInCampaign ? "quests.completed.campaign.title" : "quests.completed.quest.title");
         this.var_50.setupCampaignImage(this._window,param1,this.var_126.lastQuestInCampaign);
         var _loc3_:ITextWindow = ITextWindow(this._window.findChildByName("desc_txt"));
         var _loc4_:int = _loc3_.height;
         this.setDesc(this.var_126.getQuestLocalizationKey() + ".completed");
         _loc3_.height = Math.max(const_1469,_loc3_.textHeight + const_1470);
      ...
      {
         this.var_50.localization.registerParameter(param1,"category",this.var_50.getCampaignName(this.var_126));
      ...
      private function onCatalogLink(param1:WindowEvent, param2:IWindow = null) : void
      {
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            this.var_50.openCatalog(this.var_126);
         }
      }
      ...
      public function update(param1:uint) : void
      {
         if(this.var_1180 > 0)
         {
            ...
            if(this.var_1180 < 1)
            {
               ...
               if(this.var_126.lastQuestInCampaign)
               {
                  this.var_628.restart();
               }
               else
               {
                  this.var_628.stop();
