package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_961:int = 3;
      
      private var var_442:AssetLoaderStruct;
      
      private var var_962:Timer;
      ...
      private const const_2369:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_442 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_962 = new Timer(this.const_2369,1);
         this.var_962.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_442)
         {
            this.var_442.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_442.addEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_442)
         {
            this.var_442.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_442.removeEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_961 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1175);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_961);
            if(this.var_1175.indexOf("?") > 0)
            {
               _loc2_ = this.var_1175 + "&retry=" + this.var_961;
            }
            else
            {
               _loc2_ = this.var_1175 + "?retry=" + this.var_961;
            ...
            this.var_442 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2303,this._assets);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1741)
         {
            this.var_1176.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_596[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_596[_loc3_] = _loc6_;
            }
            this.var_963[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_963)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_596[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_963[_loc3_];
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_773.length > 0 && this.var_772.length < this.const_1828)
         {
