package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_2114:int = 60000;
       
      
      private var _structure:AvatarStructure;
      
      private var var_1569:IAvatarImage;
      ...
      private var var_400:IActiveActionData;
      
      private var var_502:AvatarImageBodyPartContainer;
      ...
      private var var_991:PaletteMap;
      
      private var var_1068:Array;
      
      private var var_1069:Point;
      ...
      public function dispose() : void
      {
         ...
         {
            _loc2_ = this.var_297.getKeys();
            for each(_loc3_ in _loc2_)
            {
               _loc1_ = this.var_297.getValue(_loc3_) as AvatarImageBodyPartCache;
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
               }
      ...
      public function disposeInactiveActions(param1:int = 60000) : void
      {
         ...
         var _loc4_:Array = this.var_297.getKeys();
         for each(_loc5_ in _loc4_)
         {
            _loc3_ = this.var_297.getValue(_loc5_) as AvatarImageBodyPartCache;
            if(_loc3_ != null)
            {
      ...
      public function resetBodyPartCache(param1:IActiveActionData) : void
      {
         var _loc2_:* = null;
         var _loc4_:* = null;
         var _loc3_:Array = this.var_297.getKeys();
         for each(_loc4_ in _loc3_)
         {
            _loc2_ = this.var_297.getValue(_loc4_) as AvatarImageBodyPartCache;
            if(_loc2_ != null)
            {
      ...
      public function setGeometryType(param1:String) : void
      {
         ...
         if(this.var_879 == GeometryType.const_1249 && param1 == GeometryType.const_1237 || this.var_879 == GeometryType.const_1237 && param1 == GeometryType.const_1249)
         {
            this.var_879 = param1;
            this.var_98 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         if(this._bodyPartCache == null)
         {
            ...
            this.var_297.add(param1,this._bodyPartCache);
         }
         var _loc3_:int = this._bodyPartCache.getDirection();
         var _loc4_:int = param2;
         this.var_400 = this._bodyPartCache.getAction();
         var _loc5_:IActiveActionData = this.var_400;
         var _loc6_:* = [];
         var _loc7_:Dictionary = new Dictionary();
         var _loc8_:Point = new Point();
         if(!(!this.var_400 || !this.var_400.definition))
         {
            if(this.var_400.definition.isAnimation)
            {
               _loc11_ = _loc3_;
               _loc12_ = this._structure.getAnimation(this.var_400.definition.state + "." + this.var_400.actionParameter);
               _loc13_ = param2 - this.var_400.startFrame;
               if(_loc12_ != null)
               {
                  _loc14_ = _loc12_.getLayerData(_loc13_,param1,this.var_400.overridingAction);
                  if(_loc14_ != null)
                  {
                     ...
                     else if(_loc14_.type == AnimationLayerData.const_517)
                     {
                        _loc3_ = _loc11_;
                     }
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1569.getFigure(),_loc5_,this.var_879,_loc3_,_loc6_,_loc7_);
            _loc10_ = new AvatarImageDirectionCache(_loc15_);
            _loc9_.updateDirectionCache(_loc3_,_loc10_);
         }
         this.var_502 = _loc10_.getImageContainer(_loc4_);
         if(this.var_502 == null)
         {
            _loc16_ = _loc10_.getPartList();
            this.var_502 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_400);
            if(this.var_502 == null)
            {
               return null;
            }
            _loc10_.updateImageContainer(this.var_502,_loc4_);
         }
         this.var_502.offset = _loc8_;
         return this.var_502;
      }
      
      public function getBodyPartCache(param1:String) : AvatarImageBodyPartCache
      {
         var _loc2_:AvatarImageBodyPartCache = this.var_297.getValue(param1) as AvatarImageBodyPartCache;
         if(_loc2_ == null)
         {
            _loc2_ = new AvatarImageBodyPartCache();
            this.var_297.add(param1,_loc2_);
         }
         return _loc2_;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_98)
         {
            this.var_98 = this._structure.getCanvas(this._scale,this.var_879);
            if(!this.var_98)
            {
               return null;
            }
            this.var_2789 = new Point(0,this.var_98.height - int(this.var_98.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1821[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_919 || _loc10_ == AvatarAction.const_416;
         var _loc12_:int = param2.length - 1;
         _loc8_ = _loc12_;
         while(_loc8_ >= 0)
         {
            this.var_137 = param2[_loc8_] as AvatarImagePartContainer;
            if(!(param1 == 7 && (this.var_137.partType == "fc" || this.var_137.partType == "ey")))
            {
               this.var_1607 = this.var_137.bodyPartId;
               this.var_200 = this.var_137.partType;
               this.var_2043 = this.var_137.partId;
               this._frame = this.var_137.getFrameIndex(param3);
               _loc5_ = param1;
               _loc7_ = false;
               if(_loc6_)
               {
                  if(_loc9_ == "wav" && (this.var_200 == "lh" || this.var_200 == "ls" || this.var_200 == "lc" || this.var_200 == "lhs"))
                  {
                     _loc7_ = true;
                  }
                  else if(_loc9_ == "drk" && (this.var_200 == "rh" || this.var_200 == "rs" || this.var_200 == "rc" || this.var_200 == "rhs"))
                  {
                     _loc7_ = true;
                  }
                  else if(_loc9_ == "crr" && this.var_200 == "rhs")
                  {
                     _loc7_ = true;
                  }
                  else if(this.var_200 == "ri")
                  {
                     _loc7_ = true;
                  }
                  else if(this.var_200 == "cp")
                  {
                     _loc7_ = true;
                  }
                  else
                  {
                     ...
                     if(this.var_137.flippedPartType != this.var_200)
                     {
                        this.var_200 = this.var_137.flippedPartType;
                     }
                  }
               }
               this.var_1606 = this._scale + "_" + this.var_137.action.assetPartDefinition + "_" + this.var_200 + "_" + this.var_2043 + "_" + _loc5_ + "_" + this._frame;
               this.var_480 = this._assets.getAssetByName(this.var_1606) as BitmapDataAsset;
               if(this.var_480 == null)
               {
                  this.var_1606 = this._scale + "_std_" + this.var_200 + "_" + this.var_2043 + "_" + _loc5_ + "_0";
                  this.var_480 = this._assets.getAssetByName(this.var_1606) as BitmapDataAsset;
               }
               if(!(!this.var_480 || !this.var_480.content))
               {
                  if(this.var_137.paletteMapId > -1)
                  {
                     this.var_991 = this._structure.renderManager.paletteManager.getPalette(this.var_137.paletteMapId);
                     if(this.var_991 != null)
                     {
                        this.var_1067 = this.var_480.content as BitmapData;
                        this.var_201 = new BitmapData(this.var_1067.width,this.var_1067.height);
                        this.var_2788 = this.var_991.colorize(this.var_1067.getPixels(this.var_1067.rect));
                        this.var_201.setPixels(this.var_201.rect,this.var_2788);
                     }
                     else
                     {
                        this.var_201 = (this.var_480.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1607 == AvatarBodyPartType.const_260 || this.var_1607 == AvatarBodyPartType.const_1921 || this.var_1607 == AvatarBodyPartType.const_2057) && this.var_137.color != null)
                  {
                     _loc16_ = 0.7;
                  ...
                  {
                     this.var_201.colorTransform(this.var_201.rect,this.var_137.color.colorTransform);
                  }
                  if(this.var_137.isBlendable)
                  {
                     this.var_201.colorTransform(this.var_201.rect,this.var_137.blendTransform);
                  }
                  this.var_1069 = this.var_480.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_61)
                     {
                  ...
                  this.var_2790 = new ImageData(this.var_201,this.var_1069,_loc7_);
                  this.var_1068.push(this.var_2790);
               }
            }
            _loc8_--;
         }
